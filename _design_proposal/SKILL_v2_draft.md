...
description: speaker-research-logリポジトリへの毎朝6時の自動レポート生成・push（v2: 事実ベース・12カテゴリ巡回・論文原文取得・テーマ重複ゲート・Obsidian Vault協調）
version: 2
---
...
- **テーマ重複ゲート**（過去21日でタグ集合の Jaccard 係数チェック）
...
## 1. 前提：GitHub API による push 方式

本タスクはサンドボックスの git clone/push に依存せず、**GitHub Contents API + PAT（Personal Access Token）** でファイルを直接作成・更新する。
...
Chrome MCP 切り替え手順・各種 GitHub API curl/JS スニペットは [付録 A] に集約（v1 から維持）。

### 認証情報

**⚠️ セキュリティ重要：PATは本ドラフト・公開リポジトリには絶対に書かない。**
スケジュールタスク実行時は、スケジュールタスクの secrets / 環境変数 / ローカルファイル
（`%APPDATA%\Claude\secrets.json` のようなVault外） から読み込む。本ドラフトでは
ハードコードを禁止する。

- PAT: `<see local secrets file — never commit to repo>`
- リポジトリ: `furusho311-art/speaker-research-log`
- API ベース: `https://api.github.com/repos/furusho311-art/speaker-research-log/contents/`

---

## 2. ステップ1: 日付確認・既存レポート確認・URL/タグデータベース読み込み

1. 今日の日付を確認（YYYY-MM-DD 形式、JST）
2. GitHub API で `reports/YYYY/MM/YYYY-MM-DD.md` が既に存在するか確認 → 200なら正常終了
3. **引用済みURLデータベース読み込み**（v1から維持）
   - `index/cited-urls-YYYY.txt`（当年）と前年を GET、URLセット化
4. **タグ・テーマデータベース読み込み（新規・必須）**
...
各論文・特許には次の frontmatter エントリを構造化（後述）：
```yaml
papers:
...
## 5. ステップ4: モード判定（feature か patrol-only か）
...
push 後、レスポンス status 201 を確認。

---

## 10. ステップ9: index/by-topic.md 更新（v1から維持・補強）

既存 by-topic.md への追記は v1 と同じ。

**v2 追加**：feature モードの日のみ追記。patrol-only モードはスキップ（by-topic.md は特集の索引なので）。

---

## 11. ステップ10: cited-urls-YYYY.txt 更新（v1から維持）
...
- [ ] 誇張禁止リストの語が本文に出現していない（自己grepチェック）
...
- 誇張表現は**自己grepで除去**。push前に必ずチェック

---

## 付録 A: GitHub API スニペット集

（v1 の §1〜§6 から bash curl / Chrome MCP javascript_tool のスニペット全文をそのまま転載 — ここでは省略。実SKILL.md化時に v1 の該当部分をコピー）

主要スニペット：
1. ファイル存在確認（GET）
2. 新規ファイル作成（PUT）
3. 既存ファイル更新（PUT + sha）
4. レポート分割蓄積パターン（本文が長い場合）
...
*SKILL v2 / draft / 2026-05-19 / 適用前にユーザー確認待ち*
